...
```{r}
#myFiles <- onFiles[1:5]
myFiles <- sample(onFiles, 2, replace = FALSE)
...
for (myOn in myFiles){
  ...
  if (first == TRUE){
    allON <- dt_train[sample(nrow(dt_train), 20000), ]
    first <- FALSE
  } else {
...
#allTrains
x <- TRUE
while (length(meta_loc$StopID) >= 1) {
  num <- meta_loc[1, 'StopID']
  num <- as.numeric(num)
  stop_trains <- meta_loc[meta_loc$StopID == num, ]
...
```{r}
myDates <- table(cut(stop_trans$DateTime, breaks = "hour"))
myDatesN <- as.data.frame(myDates)
myDatesN
plot(myDates, type="l", xlab="DateTime", ylab="Freq")
```
...
```{r}
MydatesNew_No <- subset(MydatesNew, MydatesNew$Freq != 0)

ggplot(MydatesNew_No, aes(, y=MydatesNew_No$Freq)) + geom_boxplot()
```

```{r}
counts <- 0
for(status in MydatesNew){
status <- ifelse(MydatesNew$vec >200, "Busy", ifelse(MydatesNew$vec < 200 & MydatesNew$vec >60 , "Slightly Busy", "Not Busy"))
}

mynewDates <- cbind(MydatesNew, status) 
mynewDates
```
