...
```{r}
#myFiles <- onFiles[1:5]
myFiles <- sample(onFiles, 5, replace = FALSE)
...
for (myOn in myFiles){
  ...
  if (first == TRUE){
    allON <- dt_train[sample(nrow(dt_train), 5000), ]
    first <- FALSE
  } else {
...
status <- ifelse(all_comb$Freq >= 15, "Busy", ifelse(all_comb$vec < 15 & all_comb$vec > 7 , "Slightly Busy", "Not Busy"))
}
