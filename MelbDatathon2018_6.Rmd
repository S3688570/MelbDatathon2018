...
for (file in 1:6){
  ScanOnFolder <- sub("X",mySamp,ScanOnFolderMaster)
  ScanOffFolder <- sub("X",mySamp,ScanOffFolderMaster)
  onFilesSub <- list.files(ScanOnFolder,recursive = TRUE,full.names = TRUE, pattern = "\\.txt$")
  offFilesSub <- list.files(ScanOffFolder,recursive = TRUE,full.names = TRUE, pattern = "\\.txt$")
  ...
  if (condDatON == FALSE) {
    ...
    onFiles <- c(onFiles, onFilesSub) 
    mySamp <- mySamp + 2
  }
  
  if (condDatOFF == FALSE) {
    offFiles <- offFilesSub
    condDatOFF <-  TRUE
  } else {
    offFiles <- c(offFiles, offFilesSub) 
  }
}
...
#Sample the data to obtain a smaller dataset
meta_loc2 <- meta_loc[1:3000,]
#meta_loc2 <- meta_loc[sample(nrow(meta_loc), 200), ]
```

# Determine the frequency of MYKI transactions at each stop
```{r}
