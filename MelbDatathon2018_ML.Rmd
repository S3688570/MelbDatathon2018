...
```{r echo = FALSE}
p1 <- ggplot(data, aes(x = Slope)) + geom_bar() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + xlab("Slope") + theme(axis.title.y=element_text(size=20,face="bold"))

p2 <- ggplot(data, aes(x = Thal)) + geom_bar() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + xlab("beta-Thalassemia cardiomyopathy") + theme(axis.title.y=element_text(size=20,face="bold"))

p <- plot_grid(p1, p2)
#label <- substitute(paste("Bar Chart for ", beta, "-Thalassemia"))
#title <- ggdraw() + draw_label(label, fontface='bold', size = 24)
plot_grid(title, p, ncol = 1,  rel_heights=c(0.4, 3))

```
## Imputation
The missing values in the Slope (slope of the peak exercise ST segment) and Thal ($\beta$-Thalassemia cardiomyopathy) features were initially imputed based on all features.
...
```{r}
data_imp = impute(data, target = "Goal", cols = list(Slope = imputeLearner("classif.cforest"), Thal = imputeLearner("classif.cforest")))

```
Most of the imputed values for Slope have been assigned to 'flat' and 'unsloping' while imputed values for Thal were assigned to 'normal' and 'reversible defect'.

```{r}
table(data_imp$data$Slope)
table(data_imp$data$Thal)
```

## Transformation of 'ST Depression' feature
The histogram for the 'ST Depression' feature is slightly right skewed. It was not possible to apply the box-cox transformation since the 'ST Depression' feature contained several negative values.

```{r echo = FALSE}
data_imputed <- data_imp$data
data_imputed_filt <- data_imputed %>% filter(Oldpeak != 0)

p3 <- ggplot(data_imputed, aes(x = Oldpeak)) + geom_histogram(aes(y=..density..), bins = 25, color="black", fill = "grey") + 
  stat_function(fun = dnorm, args = list(mean = mean(data_imputed_filt$Oldpeak), sd = sd(data_imputed_filt$Oldpeak)), colour = "red", size = 0.7) + geom_vline(aes(xintercept=mean(data_imputed_filt$Oldpeak)), color="blue", linetype="dashed", size=1) + labs(x = "ST depression", y = "Density") + xlim(-0.5, 6.5) + ylim(-0.2, 2) + annotate("text", x = 4, y = 1.5, label = "Original")

p4 <- ggplot(data_imputed, aes(x = "", y = Oldpeak)) + stat_boxplot(fill="slateblue", alpha=0.5, geom = "errorbar", width = 0.2) + geom_boxplot(width = 0.35, outlier.color = "blue", fill = "white") + xlab("") + ylab("ST depression") + theme(axis.text=element_text(size=10))
```

```{r echo = FALSE}
#Cubed root transformation of Oldpeak feature
data_imputed$Oldpeak <-  sign(data_imputed$Oldpeak) * abs(data_imputed$Oldpeak)^(1/3)

#Histogram of cube root transformed data
data_imputed_filt <- data_imputed %>% filter(Oldpeak != 0)
p5 <- ggplot(data_imputed, aes(x = Oldpeak)) + geom_histogram(aes(y=..density..), bins = 25, color="black", fill = "grey") + 
  stat_function(fun = dnorm, args = list(mean = mean(data_imputed_filt$Oldpeak), sd = sd(data_imputed_filt$Oldpeak)), colour = "red", size = 0.7) + geom_vline(aes(xintercept=mean(data_imputed_filt$Oldpeak)), color="blue", linetype="dashed", size=1) + labs(x = "ST depression", y = "Density") + theme_classic() + xlim(-0.5, 6.5) + ylim(-0.2, 2) + annotate("text", x = 4, y = 1.5, label = "Cube Root \nTranformed")

p6 <- ggplot(data_imputed_filt, aes(x = "", y = Oldpeak)) + stat_boxplot(fill="slateblue", alpha=0.5, geom = "errorbar", width = 0.2) + geom_boxplot(width = 0.35, outlier.color = "blue", fill = "white") + xlab("") + ylab("ST depression") + theme(axis.text=element_text(size=10))

p <- plot_grid(p3, p5, p4, p6)
label <- substitute(paste("Exercise Induced ST Depression"))
title <- ggdraw() + draw_label(label, fontface='bold', size = 24)
plot_grid(title, p, ncol = 1,  rel_heights=c(0.4, 3))
...
set.seed(1234)
n <- nrow(data_imputed)
shuffled_data <- data_imputed[sample(n), ]
```
